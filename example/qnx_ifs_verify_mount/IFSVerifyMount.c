...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        printf("Usage: %s <public_key_file> <ifs_file> <signature_file>\n", argv[0]);
        return 1;
    }
    ...
    if (msg == MAP_FAILED) {
        printf("Failed to mmap ifs file: %s\n", strerror(errno));
        return 1;
    }
    ...
    int retval;
    if (res) {
        retval = system("./mount_ifs sample_ifs.bin /sample_mount_point");
    } else {
